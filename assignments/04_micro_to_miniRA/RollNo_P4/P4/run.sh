...
function testIR {
    ...
    if [[ $j2m == 1 ]] && [[ -f tests/input/java/$name.java ]] && [[ $nc == 0 ]]; then
        java -jar tests/J_M.jar < tests/input/java/$name.java 1>tests/input/$dir/$name.microIR 
    fi
    java -jar tests/M_O.jar < tests/input/$dir/$name.microIR 1>tests/output/exec/correct/$name 
    if [[ $nc == 0 ]]
    then
        if [[ $debug == 0 ]]
        then
